...
class WC_Gateway_Payment_Gateway_Boilerplate extends WC_Payment_Gateway {
  ...
  public function __construct() {
    ...
    $this->order_button_text  = __( 'Pay with Poppy Payments', 'woocommerce-payment-gateway-boilerplate' );

    $this->method_title       = __( 'Poppy Payments', 'woocommerce-payment-gateway-boilerplate' );
    $this->method_description = __( 'Take payments via Poppy Payments.', 'woocommerce-payment-gateway-boilerplate' );
  ...
  public function checks() {
    ...
    if( version_compare( phpversion(), '5.3', '<' ) ) {
      echo '<div class="error"><p>' . sprintf( __( 'Poppy Payments Error: Poppy Payments requires PHP 5.3 and above. You are using version %s.', 'woocommerce-payment-gateway-boilerplate' ), phpversion() ) . '</p></div>';
    }

    // Check required fields.
    else if( !$this->public_key || !$this->private_key ) {
      echo '<div class="error"><p>' . __( 'Poppy Payments Error: Please enter your public and private keys', 'woocommerce-payment-gateway-boilerplate' ) . '</p></div>';
    }
    ...
    // Show message if enabled and FORCE SSL is disabled and WordPress HTTPS plugin is not detected.
      ...
      echo '<div class="error"><p>' . sprintf( __( 'Poppy Payments is enabled, but the <a href="%s">force SSL option</a> is disabled; your checkout may not be secure! Please enable SSL and ensure your server has a valid SSL certificate - Poppy Payments will only work in sandbox mode.', 'woocommerce-payment-gateway-boilerplate'), admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) . '</p></div>';
    }
  }
  ...
  public function init_form_fields() {
    $this->form_fields = array(
      'enabled' => array(
        ...
        'label'       => __( 'Enable Poppy Payments', 'woocommerce-payment-gateway-boilerplate' ),
        'type'        => 'checkbox',
        'description' => '',
        'default'     => 'no'
      ),
      'title' => array(
        ...
        'default'     => __( 'Poppy Payments', 'woocommerce-payment-gateway-boilerplate' ),
        'desc_tip'    => true
      ),
      'description' => array(
        ...
        'default'     => 'Pay with Poppy Payments.',
        'desc_tip'    => true
      ),
      'instructions' => array(
      ...
      'sandbox_private_key' => array(
        ...
        'type'        => 'text',
        'description' => __( 'Get your API keys from your Poppy Payments account.', 'woocommerce-payment-gateway-boilerplate' ),
        'default'     => '',
        'desc_tip'    => true
      ),
  ...
  public function process_payment( $order_id ) {
    ...
    if( $this->debug == 'yes' ) {
      $this->log->add( $this->id, 'Poppy Payments payment response: ' . print_r( $payment, true ) . ')' );
    }
    ...
    if( 'APPROVED' == $payment['status'] ) {
      ...
      // Add order note.
      $order->add_order_note( sprintf( __( 'Poppy Payments payment approved (ID: %s)', 'woocommerce-payment-gateway-boilerplate' ), $payment['id'] ) );

      if( $this->debug == 'yes' ) {
        $this->log->add( $this->id, 'Poppy Payments payment approved (ID: ' . $payment['id'] . ')' );
      }
      ...
      // Add order note.
      $order->add_order_note( __( 'Poppy Payments payment declined', 'woocommerce-payment-gateway-boilerplate' ) );

      if( $this->debug == 'yes' ) {
        $this->log->add( $this->id, 'Poppy Payments payment declined (ID: ' . $payment['id'] . ')' );
      }
  ...
  public function process_refund( $order_id, $amount = null, $reason = '' ) {
    ...
    if( 'APPROVED' == $refund['status'] ) {
      ...
      $order->update_status( 'refunded', __( 'Payment refunded via Poppy Payments.', 'woocommerce-payment-gateway-boilerplate' ) );
      ...
      if( $this->debug == 'yes' ) {
        $this->log->add( $this->id, 'Poppy Payments order #' . $order_id . ' refunded successfully!' );
      }
      return true;
    }
    else {
      ...
      if( $this->debug == 'yes' ) {
        $this->log->add( $this->id, 'Error in refunding the order #' . $order_id . '. Poppy Payments response: ' . print_r( $response, true ) );
      }
